...
with open("/home/jurie/cpp_projects/tests/outputs/state_test2.out","wb") as outputFile:
	...
	while True:
		try:
			...
			ids+=1	
			obj = {"msg":{"id":ids,"type":"stateCMD","instr":"fillTank"}}
			myComms.sendMsg(obj)
			...
			count = 10
			while(myComms.status['update']['status']['state']!= 'FILL' and count >0):
				time.sleep(0.1)
				count -=1
				if(debug):
			...
			t_flowSim = threading.Thread(target=flowSim.flowSim,args=(True,10))
			t_flowSim.start()
