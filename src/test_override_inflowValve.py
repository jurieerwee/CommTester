...
''''
Test in override the inflowValve
...
with open("/home/jurie/cpp_projects/tests/outputs/state_test2.out","wb") as outputFile:
	...
	while True:
		try:
			...
			#Test inflow valve open
			ids+=1	
			obj = {"msg":{"id":ids,"type":"manualCMD","instr":"openInflowValve"}}
			myComms.sendMsg(obj)
			reply = myComms.replyQ.get()
			if (reply['reply']['id']!= ids or reply['reply']['success']!= True or reply['reply']['code']!=1):
				print ('Reply Errors')
			...
			count = 10
			while(myComms.status['update']['status']['inflowValve']!= True and count >0):
				time.sleep(0.1)
				count -=1
				if(debug):
					print(myComms.status['update']['status']['state'])
			...
			ids+=1	
			obj = {"msg":{"id":ids,"type":"manualCMD","instr":"closeInflowValve"}}
			myComms.sendMsg(obj)
			reply = myComms.replyQ.get()
			if (reply['reply']['id']!= ids or reply['reply']['success']!= True or reply['reply']['code']!=1):
				print ('Reply Errors')
			...
			count = 10
			while(myComms.status['update']['status']['inflowValve']!= False and count >0):
				time.sleep(0.1)
				count -=1
				if(debug):
					print(myComms.status['update']['status']['state'])
